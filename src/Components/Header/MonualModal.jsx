...
const MonualModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen && videoRef.current) {
      ...
      videoRef.current.currentTime = 0; // Перемотка на начало
    }
  }, [isOpen]);
  ...
  return (
    ...
    >
      <div
        className={`Monual p-[10px] bg-MainColor rounded-[8px] w-[90%] md:w-[50%] lg:w-[50%] py-[30px] transform transition-all duration-500 ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-75 opacity-0 translate-y-10'}`}
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-center text-[white] text-[30px]">Monual</h2>
