        ...
        <div className="max-w-3xl mx-auto p-4">
            ...
            <div className="space-y-4">
                {faqData.map((faq, index) => (
                    <div key={index} className="border rounded-lg p-4 transition-all duration-300 ease-in-out">
                        <button
                            onClick={() => toggleFAQ(index)}
                            className="flex justify-between w-full text-left text-lg font-medium"
                        >
                            {faq.question}
                            <span>{openIndex === index ? <CiCircleMinus className="text-xl font-black mx-3" /> : <CiCirclePlus className="text-xl font-black mx-3" />}</span>
                        </button>
