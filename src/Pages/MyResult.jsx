  ...
  return (
    <div className='MyResult pt-[100px] pb-[100px] w-full'>
      <div className='Container'>
        <h1 className='text-[30px] font-bold text-center mb-[20px]'>My Result</h1>
            ...
            data.map((i) => (
              <div key={i.id} className='p-[10px] border-[2px] border-MainColor w-full rounded-[8px] MyResult__card flex items-center justify-between'>
                <div>
                  ...
                  <span className={`block text-[20px] ${i.status === 'PENDING' ? 'text-[#ff2121]' : 'text-[green]'}`}>
                    <span className='font-bold text-black'>Status:</span> {i.status}
                  </span>
                  {i.remainSection}
                  {i.status === "PENDING" && (
                    <span className='block text-[20px]'>
                      <span className='font-bold'>Time:</span> {i.countdown > 0 ? i.countdown : 'Time is up!'}s
                    </span>
                  )}
                </div>
                <div className='myResult__grid flex items-center gap-[30px] mt-[15px]'>
                  {i.status !== 'PENDING' ? (
                    ...
                    <div onClick={() => ModalActive(i)} className='rounded-[8px] bg-MainColor py-[20px] px-[5px] flex items-center justify-center w-[120px] text-center cursor-pointer'>
